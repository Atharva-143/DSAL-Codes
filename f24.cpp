...
{
    fstream data_file(data_file_name, ios::binary | ios::in | ios::out);
    fstream index_file(index_file_name, ios::binary | ios::in | ios::out);
...
void employeeDatabase::addEmployee(int eid, char name[50], char dest[50], int sal) 
{
    ...
    // Update index if the employee already existed (position was -1)
    if (update) 
    {
        index_file.open(index_file_name, ios::binary | ios::out);
        index_file.seekp(current.position, ios::beg);
        index_file.write((char *)&current, sizeof(current));
    } 
    else 
    {
        // Add new entry to the index file while maintaining order
        ...
        {
            index_file.write((char *)&pair, sizeof(index_pair));
        }
    }
...
bool employeeDatabase::isPresent(int eid) 
{
    ...
    while (index_file.read((char *)&current, sizeof(index_pair))) 
    {
        ...
        {
            index_file.close();
            return current.position != -1;  // Position should not be -1
        }
    }
    index_file.close();
    return false;
...
void employeeDatabase::deleteEmployee(int eid) 
{
    ...
    while (index_file.read((char *)&current, sizeof(index_pair))) 
    {
        ...
        {
            deleted = true;
            current.position = -1;  // Mark as deleted
        }
